...
### 动态图片
:::info
目前仅支持 XLSX 与 DOCX 文件格式。请注意：插入图片后的 XLSX 文件暂不支持转换为 PDF 格式。
:::
...
:::warn
如果在实际渲染过程中因各种原因未能获取到目标图片数据（如图片链接无效），则仍会显示占位符图片。
:::
...
### 条码
:::info
目前仅支持 XLSX 与 DOCX 文件格式。请注意：插入二维码后的 XLSX 文件暂不支持转换为 PDF 格式。
:::
