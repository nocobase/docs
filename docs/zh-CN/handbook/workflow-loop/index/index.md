...
![创建循环节点](./ca5002e1-75fc-4efd-8b6b-2284d7c2fe8a.png)
...
2.  创建工作流，触发器选择“数据表事件”，选择“订单”表“新增数据时”触发，并且需要配置上预加载“订单明细”表和明细下的商品表的关系数据：
    ...
    ![循环节点_示例_触发器配置](./76c97498-bfd6-46f4-99da-9734cb10e400.png)

3.  创建循环节点，选择循环对象为“触发数据 / 订单明细”，即对订单明细表中的每一条数据：

    ![循环节点_示例_循环节点配置](./961c20bb-f1c8-4bba-96b3-79ea74dcc66c.png)

4.  循环节点内部创建一个“条件判断”节点，判断商品的库存是否充足：

    ![循环节点_示例_条件判断节点配置](./85085520-9287-4353-b978-b47c859972fc.png)

5.  如果充足则在“是”的分支中创建一个“计算节点”和一个“更新数据”节点，将计算完扣减的库存更新至对应商品的记录：

    ![循环节点_示例_计算节点配置](./5ec4ff91-e3ff-4ee2-91d7-4602b13d605f.png)

    ![循环节点_示例_更新库存节点配置](./85c59e6d-90bb-47bd-b36a-7ecf1744a630.png)

6.  否则在“否”的分支中创建一个“更新数据”节点，更新订单明细的状态为“无效”：

    ![循环节点_示例_更新订单明细节点配置](./856f8c35-d35b-4879-81d0-3d334e6d9a42.png)

总的流程结构如下图：

![循环节点_示例_流程结构](./7cd2dfe7-6a43-43b8-90d7-9c9c323f888e.png)
