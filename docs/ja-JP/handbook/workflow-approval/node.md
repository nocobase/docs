...
承認が完了すると、関連するステータスとデータはノード結果に記録され、後続のノードで変数として使用できます。

![20250614095052](https://static-docs.nocobase.com/20250614095052.png)

### ノード承認ステータス

現在の承認ノードの処理ステータスを表します。結果は列挙値です。参照：[承認ノードの処理ステータス](./advanced.md#审批节点的处理状态)。

### 承認後のデータ

承認者がフォーム上で承認内容を修正した場合、その修正データはノード結果に記録され、後続のノードで使用されます。リレーションフィールドを使用するには、トリガーでそのフィールドのプリロードを設定する必要があります。

### 承認記録

> v1.8.0+

承認処理記録は配列であり、該当ノード内のすべての承認者の処理記録を含みます。各記録には以下のフィールドが含まれます：

| フィールド | 型 | 説明 |
| --- | --- | --- |
| id | number | 処理記録の一意の識別子 |
| userId | number | この記録を処理したユーザーのID |
| status | number | 処理ステータス。参照：[承認者の処理ステータス](./advanced.md#审批人处理记录的状态) |
| comment | string | 処理時のコメント |
| updatedAt | string | 記録の更新日時 |

必要に応じて、これらのフィールドを後続のノードで変数として使用できます。
